...
import ForwardedRadioColorGroup from './RadioColorGroup'
...
export const Product = ({ product, isShowColor = true, isShowPrice = true, isShowAddCart = true }: IProductProps) => {
        ...
        <Link to='/'>
          ...
          <div className={`absolute left-2 right-2 top-1 ${isShowAddCart ? 'group-hover:blur-sm' : ''}`}>
            <div className={`flex items-center justify-between `}>
              ...
              <IconHeart className='w-6 h-6 text-red-500' fill='red' />
            </div>
          </div>
        </Link>
        {isShowAddCart && (
              ...
              {product?.variant.map((v) => {
                if (v.variantAttributeValue?.colorId === colorData.id) {
                  return (
                    ...
                    >
                      {product.variantAttributeTypes?.size.find((s) => s.id === v.variantAttributeValue?.sizeId)?.name}
                    </Button>
                  )
                }
              })}
