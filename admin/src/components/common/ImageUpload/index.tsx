...
const ImageUpload = React.forwardRef<HTMLInputElement, ImageUploadProps>(
  (
    ...
    const [preview, setPreview] = useState<string | null>(initialImageUrl || null) // Show the initial image if provided
    ...
    return (
      <div className={`relative`}>
        ...
        <div
          className={`${isDisabled ? 'opacity-70  ' : ''} min-h-56 relative mb-5.5 block w-full cursor-pointer appearance-none rounded border border-dashed border-primary bg-gray py-4 px-4 dark:bg-meta-4 sm:py-7.5 ${className}`}
        >
          {loading ? (
            <Loader type='inside' />  
          ) : (
            <>
              ...
              <input
                ...
                id={name}
                name={name}
                className={`absolute inset-0 z-50 m-0 h-full w-full p-0 opacity-0 outline-none ${isDisabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                onChange={handleFileChange}
              />  
              <div className='flex flex-col items-center justify-center space-y-3'>
                {preview ? (
                  <img src={preview} alt='Preview' className='max-h-40 w-auto rounded-md border' />
                ) : (
                  <>
